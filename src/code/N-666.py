...
class Player():
    ...
    def indanger(self, me, allcells):  # 判断自己周围是否有和比自己大的球体对撞的风险,，并且排除两个球突然变大吃掉自己的风险，cells按距离大小排好
        ...
        while count < forward(me_copy, alist[1]):
            ...
            for x in range(len(alist)):
                ...
                if self.distance_from(me_copy,
                                      i) < i.radius + 1.3 * me_copy.radius and i.radius > me_copy.radius and i.id != self.id and self.closer(
                    me_copy, i):
        ...
        if len(alist) >= 3:  # 排除两个球突然变大吃掉自己的风险
            for cxk in range(1, min(len(alist) - 1, 18)):
                ...
                if (other1.radius ** 2 + other2.radius ** 2) ** 0.5 > me_copy.radius and self.closer(other1,
                                                                                                     other2) and ozi2 != \
                        cells[-1]:  # and d_ < 2*self.distance_from(other1, me_copy) - me_copy.radius:
                    ...
                    while i < forward(os, ob):
                        ...
                        if (1.15 * os.radius < ome.radius <= 0.9 * ob.radius) and (
                                ome.radius + os.radius >= self.distance_from(ome, os)) and (
                                ome.radius + ob.radius < self.distance_from(ome, ob)) and (
                                ob.radius + os.radius < self.distance_from(ob, os)):
                            # print("zzzzzzzzzzz")
                            break
                        elif (1.15 * ob.radius < ome.radius) and ((ome.radius + os.radius >= self.distance_from(ome,
                                                                                                                os) or ome.radius + ob.radius >= self.distance_from(
                            ome, ob))) and (ob.radius + os.radius < self.distance_from(ob, os)):
                            # print("ggggggggggg")
                            break
                    ...
                    if num != 0:
                        ...
                        vx = (ob.radius ** 2 * ob.veloc[0] + os.radius ** 2 * os.veloc[0]) / (
                                ob.radius ** 2 + os.radius ** 2)
                        vy = (ob.radius ** 2 * ob.veloc[1] + os.radius ** 2 * os.veloc[1]) / (
                                ob.radius ** 2 + os.radius ** 2)
    ...
    def chi(self,me,other):
        d1=me.pos
        d2=other.pos
        vector1=[d2[0]-d1[0],d2[1]-d1[1]]
        d21=[other.pos[0]+other.veloc[0]*Consts["FRAME_DELTA"]*10,other.pos[1]+other.veloc[1]*Consts["FRAME_DELTA"]*10]
        d11=[me.pos[0]+me.veloc[0]*Consts["FRAME_DELTA"]*10,me.pos[1]+me.veloc[1]*Consts['FRAME_DELTA']*10]
        vector2=[d21[0]-d11[0],d21[1]-d11[1]]
        theta1=math.atan2(vector2[0],vector2[1])-math.atan2(vector1[0],vector1[1])
        v=self.veloc_from(other,me)
        dis=self.disxy_from(me,other)
        theta1=theta1*5
        print(theta1)
        return math.atan2(dis[0],dis[1])+theta1+math.pi
    ...
    def loss(self, sel, cel):
        ...
        if jiao2 < jiao1:
            ...
            return abs(jiao1 - jiao2) * norm(rel_vel) / con_veloc * (1 + norm(rel_vel) ** 2) + 3.4  # 返回大概喷多少次，大概的数字

    def price(self, sel, cel):  # 返回吃球之后的增大的倍数，还是估计值，返回值要大于1.1倍，因为是估计的值
        # nonlocal Consts
        rate = Consts["EJECT_MASS_RATIO"]
        sun = self.loss(sel, cel)
        if (1 - rate) ** sun < 1.075 * (cel.radius / sel.radius) ** 2:
            return 0
        else:
    ...
    def chitime(self, sel, cel):  # 模拟吃球的估计时间：喷球的时间+（相对距离-两个球半径之和）/相对速度 这是一个先喷后滑行吃的过程的时间，不可能一直喷球加速吃，太浪费
        ...
        rel_vel = [sel.veloc[0] - cel.veloc[0], sel.veloc[1] - cel.veloc[1]]
        peng = norm(dis) - r1 - r2
        t1 = peng / (max(norm(rel_vel), 0.4))  # 滑行时间
        t2 = self.loss(sel, cel)  # 喷球时间
        return t1 + t2

    def dianz(self, a, b, c):  # 3坐标a,b,c,返回a到直线bc的距离
        u1 = [c[0] - b[0], c[1] - b[1]]
        u2 = [a[0] - b[0], a[1] - b[1]]
    ...
    def strategy(self, allcells):
        ...
        for each in cells:
            ...
            if each.radius <= me.radius * 0.1:  # 如果半径比小于0.2，说明目标球体比喷射出的还小，不需要判断
                continue
            if self.distance_from(me, each) > km * (me.radius + each.radius):  # 星体离自己太远、不判断
                continue
            '''else:
                print(self.price(me, each))
                for x in big:
                    rel_big=self.disxy_from(me,x)
                    rel_cel=self.disxy_from(me,each)
                    a1=[rel_cel[0]+me.pos[0],rel_cel[1]+me.pos[1]]
                    b1=[rel_big[0]+me.pos[0],rel_big[1]+me.pos[1]]
                    mp=me.pos
                    if self.dianz(b1,mp,a1) < me.radius + x.radius +70 and rel_big[0] * rel_cel[0] + rel_cel[1] + rel_big[1] >0 and  rel_big[0] * rel_cel[0] + rel_cel[1] + rel_big[1] < self.norm(rel_cel)**2:
                            break
                    else:'''

            if each.radius < me.radius:
                if self.price(me, each) > 1.07:
                    prepare.append(each)
        final = sorted(prepare, key=lambda cell: self.price(me, cell) / abs(self.chitime(me, cell))**0.5)
        if len(final) == 0:
            return None
        else:
            target = final[-1]
            v_r = [me.veloc[0] - target.veloc[0], me.veloc[1] - target.veloc[1]]
            if self.loss(me, target) > 0 or norm(v_r) < 0.4:
                return self.chi(me, target)
