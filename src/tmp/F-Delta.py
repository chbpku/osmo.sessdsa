...
class Player():
    ...
    def indanger(self, me, allcells):  # 判断自己周围是否有和比自己大的球体对撞的风险,，并且排除两个球突然变大吃掉自己的风险，cells按距离大小排好
        ...
        while count < forward(me_copy, alist[1]):
            me_copy.pos[0] += me_copy.veloc[0] * Consts["FRAME_DELTA"]
            me_copy.pos[1] += me_copy.veloc[1] * Consts["FRAME_DELTA"]
            me.pos[0] = me.pos[0] % Consts["WORLD_X"]
            me.pos[1] = me.pos[1] % Consts["WORLD_Y"]
            for x in range(len(alist)):
                i = alist[x]
                i.pos[0] += i.veloc[0] * Consts["FRAME_DELTA"]
                i.pos[1] += i.veloc[1] * Consts["FRAME_DELTA"]
            ...
            for cxk in range(1, min(len(alist) - 1, 18)):
                ...
                if (other1.radius ** 2 + other2.radius ** 2) ** 0.5 > me_copy.radius and self.closer(other1,
                    ...
                    while i < forward(os, ob):
                        ob.pos[0] += ob.veloc[0] * Consts["FRAME_DELTA"]
                        ob.pos[1] += ob.veloc[1] * Consts["FRAME_DELTA"]
                        ob.pos[0] = ob.pos[0] % Consts["WORLD_X"]
                        ob.pos[1] = ob.pos[1] % Consts["WORLD_Y"]
                        os.pos[0] += os.veloc[0] * Consts["FRAME_DELTA"]
                        os.pos[1] += os.veloc[1] * Consts["FRAME_DELTA"]
                        os.pos[0] = os.pos[0] % Consts["WORLD_X"]
                        os.pos[1] = os.pos[1] % Consts["WORLD_Y"]
                        ome.pos[0] += ome.veloc[0] * Consts["FRAME_DELTA"]
                        ome.pos[1] += ome.veloc[1] * Consts["FRAME_DELTA"]
                    ...
                    if num != 0:
                        ...
                        while j < forward(ome, ob) * 0.6:
                            ob.pos[0] += vx * Consts["FRAME_DELTA"]
                            ob.pos[1] += vy * Consts["FRAME_DELTA"]
                            ob.pos[0] = ob.pos[0] % Consts["WORLD_X"]
                            ob.pos[1] = ob.pos[1] % Consts["WORLD_Y"]
                            ome.pos[0] += ome.veloc[0] * Consts["FRAME_DELTA"]
                            ome.pos[1] += ome.veloc[0] * Consts["FRAME_DELTA"]
                            ome.pos[0] = ome.pos[0] % Consts["WORLD_X"]
                            ome.pos[1] = ome.pos[1] % Consts["WORLD_Y"]
    ...
    def price(self, sel, cel):  # 返回吃球之后的增大的倍数，还是估计值，返回值要大于1.1倍，因为是估计的值
        ...
        if (1 - rate) ** sun < 1.0234 * (cel.radius / sel.radius) ** 2:
            return 0
        else:
    ...
    def strategy(self, allcells):
        ...
        for each in cells:
            ...
            if self.distance_from(me, each) > 5 * (me.radius + each.radius):  # 星体离自己太远、不判断
                ...
                for x in big:
                    ...
                    mp=me.pos
                    if self.dianz(b1,mp,a1) < me.radius + x.radius +80 and rel_big[0] * rel_cel[0] + rel_cel[1] + rel_big[1] >0 and  rel_big[0] * rel_cel[0] + rel_cel[1] + rel_big[1] < self.norm(rel_cel)**2:
                            break
                    else:

                        if each.radius < me.radius :
                            if self.price(me, each) > 1.1:
                                prepare.append(each)
        ...
        if len(final) == 0:
            ...
            if self.loss(me, target) > 0 or norm(v_r) < 0.312:
