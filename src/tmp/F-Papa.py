...
class Player():
    ...
    def strategy(self, allcells):
        ...
        def danger_condition(sel,hunter):
            ...
            #不要让速度过快
            if ((abs(rx)>min(abs(rx + 1000), abs(rx - 1000)) or abs(ry)>min(abs(ry + 500), abs(ry - 500)))) and space<3*(hunter.radius+sel.radius):
                ...
                if vx1 * math.cos(posangle) + vy1 * math.sin(posangle) >=(vx2 * math.cos(posangle) + vy2 * math.sin(posangle)) and space<1.15**(hunter.radius+sel.radius):
                    if math.sqrt(vx2 ** 2 + vy2 ** 2) > 30 or math.sqrt(vx2 ** 2 + vy2 ** 2) > 1.6 * allcells[
                        self.id].radius ** 0.7:
                        return math.atan2(-rx, -ry)+math.pi*0.5
                    else:
        ...
        def attack(sel,cel,choice):  # 吃球函数
            ...
            if choice==1:
                if math.sqrt(vx2 ** 2 + vy2 ** 2) > 30 or math.sqrt(vx2 ** 2 + vy2 ** 2) > 1.6 * allcells[
                    self.id].radius ** 0.7:
                    return None
                if shouyi(sel,cel) > 0.8:
                    return t
                else:
                    return None
            if choice==2:
                if shouyi(sel,cel) > 0.9 and survive()>20:
                    return t
                elif shouyi(sel,cel) > 1.1 and survive()<=20:
                    return t
                else:
                    return None
            if choice==3:
        ...
        while j:
            if j >= len(allcells):
                return None
            mindistance_cell = distancelist[j]
            if (mindistance_cell.radius < 0.2 * allcells[self.id].radius)  or mindistance_cell.dead==True:
                ...
                return attack(allcells[self.id],allcells[opid],3)
            elif survive()>30 and mindistance_cell.radius <0.9* allcells[self.id].radius and mindistance_cell.radius >0.2* allcells[self.id].radius or mindistance_cell.radius>4:
                    return attack(allcells[self.id],mindistance_cell,1)
            elif survive()<=30:
                editallcells=copy(allcells)
                editallcells.remove(allcells[self.id])
                i=sorted(editallcells,key=lambda cell: shouyi(allcells[self.id],cell))[-1]
                if allcells[self.id].radius*shouyi(allcells[self.id],i)>1.2*i.radius:
                    return attack(allcells[self.id],i,2)
            j += 1
