...
class Player():
    ...
    def two_cells(self,cells,selfcell):# 对于周围有两个大球的决策情况
    ...
    def strategy(self, allcells):
        ...
        for i in range(0,len(allcells)):
            if allcells[i].radius > allcells[self.id].radius and i!= self.id:
                ...
                if dir1 > 0 and dir1 < 0.5*math.pi and distance1 < 50: #z找出运动方向可能和我们相交的大球
                    if md == 0:
                        ...
                        if distance1 <(self.distance_from(bigcells[0],allcells[self.id])-bigcells[i].radius-allcells[self.id].radius):#与第一个球比较，如果距离我们更近就插入到0位置
                            ...
                            bigcells.insert(md,temp)
                        else:#如果当前球没有0近，就插最后
                            bigcells.append(allcells[i])
            if allcells[i].radius < allcells[self.id].radius and i!= self.id and allcells[i].radius > 0.3*allcells[self.id].radius:
                ...
                if distance2 < 50 and dir2 > math.pi/6 and dir2 < math.pi/3:
       ...
       #吃球函数首先只能吃当前运动方向范围内的小球，不能反向吃球，损失太大，其次要吃一定大小范围内的球，
        ...
        if len(bigcells) == 0 and len(smallcells) == 0:
            return None
        elif len(smallcells) == 0 and len(bigcells) > 0:                 #躲球加pi尝试，可删除
            if len(bigcells) == 1 and (self.distance_from(allcells[self.id], bigcells[0]) - allcells[i].radius - allcells[self.id].radius) <5:
                return self.direction2(bigcells[0], allcells[self.id]) 
            elif len(bigcells) == 2 and (self.distance_from(allcells[self.id], bigcells[0]) - allcells[i].radius - allcells[self.id].radius) <5:
                return self.two_cells(bigcells, allcells[self.id])
            else:
                if (self.distance_from(allcells[self.id], bigcells[0]) - allcells[i].radius - allcells[self.id].radius) <5:#闪躲离我们最近的球
                    return self.direction2(bigcells[0], allcells[self.id])
                else:
                    return None

        elif len(bigcells) == 0 and len(smallcells) > 0:
            if smallcells[-1].radius > 10*(allcells[self.id].radius * math.sqrt(Consts["EJECT_MASS_RATIO"])):
                return None
            else:
